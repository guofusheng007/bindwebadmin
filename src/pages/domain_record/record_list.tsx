...
import AddRecordForm from './add_record'
...
const App: React.FC = () => {
  ...
  const columns: TableProps<Item>['columns'] = [
    ...
    },
    {
      ...
      render:((value, record, index)=>{
        ...
        return (<>
            {
              (value.length == 0) ? (<div></div>)
              :(
                <Typography.Text  style={{ margin: 0 }} copyable ellipsis>
                {value}
              </Typography.Text>
              )
            }
    ...
    },
    {
      ...
      render:((value, record, index)=>{
          ...
          const onClickDelete = async () => {
            ...
            //UpdaterecordInfo(jsonData,"setUpdate(GetStringRand(4))")
          ...
          const onEdit = () => {
            ...
            //console.log("父表单中jsonData Edit:",jsonData)
            NiceModal.show(
              AddRecordForm, 
                {
                    appName:<><Space><EditOutlined />Edit record</Space></>,
                    onOkClick: onOkHandle,    //向子组件传递事件
                    defaultForm:jsonData,
                    domainFlag:"null",
                }
            )
          }
          return (<>
            <Space>
              <Typography.Link onClick={onEdit}>Edit</Typography.Link>
  ...
  //url取参，用于识别域名列表中记录的Action档位跳转过来的页面。
  const [params] = useSearchParams()                   
  //此变量用来存储域名列表中记录的Action档位跳转过来时的域名  
  //null表示非跳转方式
  //跳转过来时，此变量为域名。
  const [domainFlag, setDomainFlag] = useState("null");  
  //屏幕大小识别
  ...
  //当打开页面时，查找用户名下的域名数量
  const [domainNum, setDomainNum] = useState(0); 
  //table更新时的状态值，此值有变化时将会更新table表。
  const [update, setUpdate] = useState("");
  //useEffect事件
  useEffect(() => {
    console.log("record_list.tsx[]",(new Date()).toLocaleString())
    //setPathinfo(location.pathname)
    //用于识别是否从域名列表中跳转而来。
    ...
    //提取PromiseResult
      total.then((PromiseResult:any)=>{
        console.log("Promise的数据:",PromiseResult.domain)
        setDomainNum(PromiseResult.domain)
    })
  ...
  //在组件挂载后获取数据库中的初始值
  useEffect(() => {
    console.log("record_list.tsx[update]",(new Date()).toLocaleString())
    //const url = 'http://192.168.3.110:8080/updateuserinfo/'+ userid
    setLoading(true);
    const url = ShareInfo.http_api + 'recordapi/' + username + '/' + domainFlag
    axios({
      url:url,
  ...
  return(
    <>
      <Space>
        <NiceModal.Provider>
          {
            //若用户名下域名数量为0，则新增记录的button无效
            (domainNum >0) ?
            (<Button onClick={handleAdd} type="primary" style={{ marginBottom: 16 }} >Add record</Button>)
            : (<Button onClick={handleAdd} type="primary" style={{ marginBottom: 16 }} disabled>Add record</Button>)
          }
          
        </NiceModal.Provider>
        <Button type="primary" style={{ marginBottom: 16 }} disabled>Delete record</Button>
      </Space>
      <Table  
        columns={columns}   
